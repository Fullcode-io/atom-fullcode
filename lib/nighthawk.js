    ...
    this.tokenUnsubscribe = fb.auth().onIdTokenChanged(async user => {
      if (user) {
        ...
        if (!this.subscriptions.add) {
          ...
          this.userProjectsRef = fb.database().ref(`/users/${user.uid}/projects`)
          this.userProjectsRef.on('value', async (snap) => {
            let knownProjectIds = snap.val() ? Object.keys(snap.val()) : []
            this.knownProjects = await Promise.all(
              knownProjectIds.map(id => fb.database().ref(`/projects/${id}`).once('value').then(snap => snap.val()))
            )
