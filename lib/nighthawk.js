...
const Nighthawk = {
  ...
  async install_script() {
    let scriptCall = `${os.EOL}# This asks to start nighthawk in current process if not already running${os.EOL}. ${this.homeDir}/.nighthawk/nighthawk.sh${os.EOL}alias nighthawk=". $HOME/.nighthawk/nighthawk.sh"${os.EOL}`
    try {
      await fs.ensureDir(`${this.nighthawkDir}/logs`)
      await fs.copy(`${this.systemInfo.dirname}/bash-scripts/nighthawk.sh`, `${this.nighthawkDir}/nighthawk.sh`)
  ...
  setupEventHandlers() {
    ...
    atom.workspace.observeTextEditors( (editor) => {
      ...
      buffer.onWillSave( async (e) => {
        let file = buffer.file;
        let filePath = file ? file.path : e.path;
        let shouldIgnore = false
        let gitInfo = getRepoInfo(this.systemInfo.dirname)
        let gitRoot = gitInfo.root
        // this should possibly be moved into the worker
        if (gitRoot) {
          if (await fs.pathExists(`${gitRoot}/.gitignore`)) {
            let gitIgnore = await fs.readFile(`${gitRoot}/.gitignore`, {encoding: 'utf8'})
        ...
        if (buffer.isModified() && !shouldIgnore) {
