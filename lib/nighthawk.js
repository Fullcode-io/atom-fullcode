...
var DemoPackage = {
  ...
  setupEventHandlers: function () {
    ...
    setInterval(function () {
      lockFile.lock(the.lockPath, { stale: 5000 }, function (err) {
        if (err) {
          // console.log(err.code);
          if (err.code != 'ELOCKED' && the.tail) {
        ...
        console.log('Lock file missing or expired.. taking over log tail');
        tailErrorLog();
      });
    }, 5000);
    ...
    atom.workspace.observeTextEditors(function (editor) {
      ...
      buffer.onWillSave(function (e) {
        ...
        if (buffer.isModified()) {
          ...
          // let origContent = file ? fs.readFileSync(file.path, 'utf-8') : '';
          // let newDiff = diff.createPatch(filePath, origContent, buffer.getText());

          console.log('save detected.. sending pulse ', secondsSinceSave);

          recordSaveEvent(filePath, Math.round(secondsSinceSave), buffer.getText());
...
recordSaveEvent = function (filePath, secondsSinceSave, content) {
...
tailErrorLog = function () {
  ...
  var logCacher = setInterval(function () {
    console.log('tailing');
    if (lines.length) {
      var payload = { type: 'logs', data: lines, userID: 'luiz-n', timestamp: batchDate };
      streamToKinesis(payload);
      lines = [];
      batchDate = null;
    } else if (!the.tail) {
