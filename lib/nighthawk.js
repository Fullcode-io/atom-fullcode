  ...
  async install_script() {
    ...
    try {
      ...
      if (this.systemInfo.platform === "darwin") {
        // MacOS doesn't call .bashrc by default when opening up a new terminal session but
        ...
        await fs.ensureFile(`${this.homeDir}/.bash_profile`)
        let bashProfile = await fs.readFile(`${this.homeDir}/.bash_profile`, {encoding: 'utf8'})
        let scriptCall = `${os.EOL}# Nighthawk requires .bashrc to be sourced for auto-starting in MacOS${os.EOL}[ -s $HOME/.bashrc ] && . $HOME/.bashrc${os.EOL}`

        if (!bashProfile.includes(scriptCall) && !bashProfile.includes("/.bashrc")) {
          await fs.appendFile(`${this.homeDir}/.bash_profile`, scriptCall)
        }
      }
      return true
    } catch (e) {
