...
const saveCompresser = async (input, messageHandler, progress) => {
  ...
  if (pendingSaveEvent) {
    ...
    pendingSaveEvent.timeout = setTimeout(sendSaveEvent, 3000, pendingSaveEvent)
  }
  else if (input.clientInfo) {
    ...
    if (trackedProject && !isSleeping) {
      ...
      saveEvent.timeout = setTimeout(sendSaveEvent, 3000, saveEvent)
...
// pass in relativePath from main thread
  ...
  if (repoInfo.gitIgnore) {
    ...
    let relativePath = filePath.split(`${repoInfo.root}/`).pop() || filePath
