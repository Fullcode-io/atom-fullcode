...
const sendLogs = debounce( () => {
  ...
  Object.keys(logPathGroups).forEach(logPath => {
    ...
    if (knownProject) {
      ...
      payloadProjects[projectName] = payloadProjects[projectName] || { projectID: knownProject.fbID, branches: knownProject.branches, name: projectName, lines: [], path: logPathGroup.path }
      repoInfo.remoteOrigin = remoteOrigin.sync(`${repoInfo.root}/.git/config`)
  ...
  if (Object.keys(payloadProjects).length) {
    ...
    // fetch(`https://us-central1-fullcode-1.cloudfunctions.net/auth/log-event`,
    { method: 'POST',
      headers:
      {
