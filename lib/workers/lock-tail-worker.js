...
const sendLogs = debounce( () => {
  ...
  Object.keys(logPathGroups).forEach(logPath => {
    ...
    if (knownProject) {
      ...
      payloadProjects[projectName] = payloadProjects[projectName] || { fbID: knownProject.fbID, branches: knownProject.branches, name: projectName, lines: [], path: logPathGroup.path }
      repoInfo.remoteOrigin = remoteOrigin.sync(`${repoInfo.root}/.git/config`)
