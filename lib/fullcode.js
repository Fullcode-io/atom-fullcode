...
const FullCode = {
  ...
  async activate (state) {
    ...
    fb.auth().onIdTokenChanged(async user => {
      ...
      console.log('idTokenChanged Error: ', err)
      console.warn('idTokenChanged Error: ', err)
    })
  },
  ...
  async getClientInfo() {
    ...
    return {
      token: await this.user.getIdToken().catch(e => {}),
      fbID: this.user.uid,
  ...
  updateStatusIcon(message) {
    ...
    if (message) {
      ...
      if (foundProject && !isSleeping) {
        ...
        this.tooltip = atom.tooltips.add(this.tile.item, {title: `Tracking save events for project: ${message.response.searchableName}`})
      }
      else {
        ...
        let title = `Not tracking save events. No project found.`
        if (isSleeping) {
