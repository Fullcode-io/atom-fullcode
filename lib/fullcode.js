  ...
  setupStatusBar() {
    ...
    this.selectListView = new SelectListView({
      ...
      didConfirmSelection: (selection) => {
        ...
        fb.database().ref(`auth_data/${this.user.uid}`).update({ignoreEventsUntil})
        console.log('auth_data updated')
      },
      didCancelSelection: () => {
        this.cancel()
      }
  ...
  updateStatusIcon(message) {
    ...
    if (message) {
      ...
      if (foundProject && !isSleeping) {
        ...
        this.tooltip = atom.tooltips.add(this.tile.item, {title: `Currently tracking project: ${message.response.searchableName}`})
      }
      else {
        ...
        let title = `This current directory is not whitelisted for tracking.`
        if (isSleeping) {
