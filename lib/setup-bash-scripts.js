...
async function install() {
  ...
  try {
    ...
    // nighthawk.sh has to run inside of bashrc. So in order for nightawk to auto start we must
...
export function install_script() {
  ...
  return isCompatiable && promiseRetry({retries: 2, randomize: true}, (retry, num) => {
      return install()
      .catch((err) => {
        console.warn(`${err.code} error trying to install nighthawk bash scripts... trying ${3 - num} more times`)
        retry(err);
      });
  })
